...
static void place(void *, size_t asize);
...
int mm_init(void)
{
    ...
    PUT(heap_listp + (1*WSIZE), PACK(4*DSIZE, 1)); // prolog -> double word size & allocated 1
    ...
    PUT(heap_listp + (4*WSIZE), PACK(4*WSIZE, 1)); // prolog footer
    PUT(heap_listp + (5*WSIZE), PACK(0, 1)); // epilogue header

    // free_listp = heap_listp + 2*WSIZE;
    heap_listp += 2*WSIZE; // * 수정 -> 0509 2300
...
{
    void * bp;
    for(bp = NEXT_FREE(heap_listp); !GET_ALLOC(HDRP(bp)); bp = NEXT_FREE(bp)){
...
// {
//     NEXT_FREE(p) = NEXT_FREE(heap_listp);
//     PREV_FREE(p) = heap_listp;
//     PREV_FREE(NEXT_FREE(heap_listp)) = p;
//     NEXT_FREE(heap_listp) = p;
// }
